...
def plot_mode(top_n=5, worst_n=5, top_k_per_algo=2):
    ...
    for time_dir in os.listdir(results_root):
        ...
        for test_name in os.listdir(time_dir_path):
            ...
            df["param_str"] = df["model_params_dict"].apply(lambda d: make_param_str(d, max_len=100))
            ...
            for algo_name in algos:
                ...
                for col, metric_title in metrics:
                    ...
                    plt.title(f"{algo_name} (Top {top_n} by test_accuracy) - {metric_title}\n{test_name}")
                    ax.set_xticks([])
                    ax.set_xlabel("")
                    if "accuracy" in col:
                    ...
                    plt.tight_layout()
                    out_png = os.path.join(plot_subdir, f"{algo_name}_{col}_best{top_n}.png")
                    ...
                    sns.barplot(data=temp_worst, x="algorithm", y=col, hue="param_str",
                                dodge=True, width=0.8, zorder=3)
                    plt.title(f"{algo_name} (Worst {worst_n} by test_accuracy) - {metric_title}\n{test_name}")
                    ax.set_xticks([])
                    ax.set_xlabel("")
                    if "accuracy" in col:
                        plt.yticks(np.linspace(0, 1, 21))
                    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', title="Parameters")
                    plt.tight_layout()
                    out_png = os.path.join(plot_subdir, f"{algo_name}_{col}_worst{worst_n}.png")
                    plt.savefig(out_png, dpi=150)
                    plt.close()
            ...
            ax.set_axisbelow(True)
            plt.grid(True, which='both', linestyle='--', linewidth=0.5, zorder=0)
            sns.barplot(data=top_all, x="algorithm", y="test_accuracy", hue="param_str",
                        dodge=True, width=0.8, zorder=3)
            ...
            ax.set_axisbelow(True)
            plt.grid(True, which='both', linestyle='--', linewidth=0.5, zorder=0)
            sns.barplot(data=worst_all, x="algorithm", y="test_accuracy", hue="param_str",
                        dodge=True, width=0.8, zorder=3)
            ...
            plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', title="Parameters")
            plt.tight_layout()
            out_png = os.path.join(plot_subdir, f"global_worst{worst_n}_test_accuracy.png")
            plt.savefig(out_png, dpi=150)
            plt.close()
            ...
            ax.set_axisbelow(True)
            plt.grid(True, which='both', linestyle='--', linewidth=0.5, zorder=0)
            sns.barplot(data=comp_df, x="algorithm", y="test_accuracy", hue="param_str",
                        dodge=True, width=0.8, zorder=3)
            ...
            plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', title="Parameters")
            plt.tight_layout()
            out_png = os.path.join(plot_subdir, f"comparison_top{top_k_per_algo}_per_algo.png")
            plt.savefig(out_png, dpi=150)
            plt.close()
            
            # Comparison: Variant 2 (x=param_str, hue=algorithm)
            plt.figure(figsize=(24, 8))
            ax = plt.gca()
            ax.set_axisbelow(True)
            plt.grid(True, which='both', linestyle='--', linewidth=0.5, zorder=0)
            sns.barplot(data=comp_df, x="param_str", y="test_accuracy", hue="algorithm",
                        dodge=True, width=0.8, zorder=3)
            ...
            # Line plots: dependency of test_accuracy on each hyperparameter per algorithm,
            # starting from the worst accuracy (no confidence interval)
            params_to_compare = ["n_estimators", "learning_rate", "depth", "c", "convergence_criterion"]
            for param in params_to_compare:
                plt.figure(figsize=(24, 8))
                for algo in df["algorithm"].unique():
                    sub_df = df[df["algorithm"] == algo].copy()
                    sub_df[param] = sub_df["model_params_dict"].apply(lambda d: d.get(param, np.nan))
                    sub_df = sub_df.dropna(subset=[param])
                    if sub_df.empty:
                        continue
                    sub_df = sub_df.sort_values(by=param)
                    worst_val = sub_df["test_accuracy"].min()
                    sns.lineplot(data=sub_df, x=param, y="test_accuracy", marker="o",
                                 label=algo, ci=None)
                    plt.ylim(worst_val, 1)
                ...
                plt.tight_layout()
                out_png = os.path.join(plot_subdir, f"line_accuracy_vs_{param}.png")
                plt.savefig(out_png, dpi=150)
                plt.close()
            
            # Confusion Matrices for global top & worst
            ...
            y_test = test_data[:, -1]
            cm_df = pd.concat([top_all, worst_all]).drop_duplicates(subset=["algorithm", "file_postfix", "param_str"])
