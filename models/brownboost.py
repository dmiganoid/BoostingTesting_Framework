...
class BrownBoostClassifierGPU:
    def __init__(self, estimator=None, c=4, convergence_criterion=0.001, max_estimators=200000, random_state=None):
        self.estimator = estimator
        self.c = c
        self.max_estimators = max_estimators
        self.max_iter_newton_raphson = 100
    ...
    def get_params(self, deep=True):
        return {"convergence_criterion": self.convergence_criterion, "c": self.c, "max_estimators": self.max_estimators, "estimator": self.estimator, "random_state": self.random_state}

    def set_params(self, **p):
        self.estimator = p.get("estimator", self.estimator)
        self.c = p.get("c", self.c)
        self.max_estimators = p.get("max_estimators", self.max_estimators)
